...
class OutgoingConnectionMessaging extends Actor with ActorLogging {
...
object OutgoingConnectionMessaging {
  def props() = Props(classOf[OutgoingConnectionMessaging])
}
